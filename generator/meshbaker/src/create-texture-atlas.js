...
export const createTextureAtlas = async ({ meshes, atlasSize = 4096 }) => {
    ...
    const IMAGE_NAMES = ["diffuse", "normal", "orm", "emissive"];
    const bakeObjects = [];
    ...
    bakeObjects.forEach((bakeObject) => {
        ...
        IMAGE_NAMES.forEach((name) => {
            ...
            if (image !== '' && image !== undefined) {
                    ...
                    const arr = createImageData(imageData, image.width, image.height);
                ...
                context.fillStyle = name === 'diffuse' ? `#${material.color.clone().getHexString()}` : name === 'normal' ? '#8080ff' : name === 'orm' ?
                    `#${(new THREE.Color(1, material.roughness, material.metalness)).getHexString()}` : name === 'emissive' ? '#000000' : '#7F7F7F';
    ...
    const textures = Object.fromEntries(await Promise.all(IMAGE_NAMES.map(async (name) => {
      // write contexts[name].canvas to contexts[name].jpg
      const out = fs.createWriteStream(`./${name}.jpg`);
      const stream = contexts[name].canvas.createJPEGStream();
      stream.pipe(out);
    ...
    const IMAGE_NAMES = ["diffuse", "normal", "orm"];
      ...
      bakeObjects.forEach((bakeObject) => {
        ...
        IMAGE_NAMES.forEach((name) => {
          ...
          // iterate through imageToMaterialMapping[name] and find the first image that is not null
          let data = getTextureImage(material, imageToMaterialMapping[name].find((textureName) => getTextureImage(material, textureName)));

          if (data) {
            const imageData = new Uint8ClampedArray(data.data);
            const arr = createImageData(imageData, data.width, data.height);
